  ...
  return (
    <article className={styles.block} {...props}>
      <p onClick={handleClick}>
        {getDateFromStoryblokFormat(dateObj)} - {getDateFromStoryblokFormat(dateObj.setDate(dateObj.getDate() + 7))}
      </p>
      <AnimatePresence>
        {active &&
          <RichTextDangerouslyInnerHTML initial={{height: 0, margin: 0}} animate={{height: 'auto', margin: '1rem'}}
                                        exit={{height: 0, margin: 0}} styling={styles.text} richText={text}/>}
      </AnimatePresence>
    </article>
  );
}
